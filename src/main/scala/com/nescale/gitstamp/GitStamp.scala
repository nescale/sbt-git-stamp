/*
 * Copyright 2019-2022 HM Revenue & Customs
 * Copyright 2022 Nescale
 *
...
object GitStamp {
  ...
  def gitStamp(repository: Repository): Map[String, String] = {
    ...
    Map(
      "Build-Date"        -> LocalDateTime.now().toString,
  ...
  private def commitDateTime(headRev: Option[RevCommit]): String = {
    val value = headRev.map(_.getCommitTime.toLong * 1000).getOrElse(0L)
    value.toString
  }
